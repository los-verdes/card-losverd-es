...
| `status` | `'active'` if the (merged) `expiration_date >= today`, else `'expired'`; an existing `'revoked'` is left unchanged | Mirrors `AnnualMembership.is_active` (created_on within the last 365 days) across *all* of a member's orders, not just the one being synced. Revocation (`'revoked'`) is out of scope for order sync — it's an admin action, not something BigCommerce order data expresses — so a sync never sets it and never clears it either. |
