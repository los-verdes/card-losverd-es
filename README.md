...
- **New-order card emails** (`src/email/newOrder.ts`): when an order webhook reports an order has reached `Completed`, the member is emailed their card, once. On in both environments, for orders created on or after `CARD_EMAIL_NEW_ORDERS_SINCE`; emptying that var switches it off -- see [`docs/bigcommerce-ingestion.md`](docs/bigcommerce-ingestion.md).
