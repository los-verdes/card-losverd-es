...
- **`etl-sync` queue** (`src/queues/`): one consumer at a time, five retries, then a dead-letter queue, whose consumer posts one Slack alert per batch. `just queue-dlq-drill [env]` proves that path end to end in a real environment by sending a message no handler understands; it takes about thirteen minutes, nearly all of it retry backoff. Carries BigCommerce order syncs and the scheduled jobs below.
