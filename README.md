...
| `/admin/orders/:id` | One membership order; attribute it to someone other than its purchaser, with an audit trail, or re-read it from BigCommerce (`POST .../reread`, never emails) (`src/admin/orders.tsx`) | Admin |
...
- **Scheduled jobs** (`src/scheduled.ts`), which `just etl-run <env> <job>` can also trigger on demand -- it enqueues onto the same queue the cron uses, so the job runs exactly as it does on a timer: BigCommerce order resync, the Slack members sync, a daily refresh of the wallet passes of memberships that lapsed the day before (`src/member/passExpirySweep.ts`; `just etl-run <env> refresh-lapsed-passes` is its one-off companion for passes that lapsed before it existed), an hourly watch that measures a handful of operational signals -- unhandled errors, devices reporting pass failures, and jobs that have stopped running -- and posts to Slack only when one keeps firing (`src/ops/watch.ts`: two runs in a row before it says anything, then at most once a day, and once more when it clears; every signal is on `/admin/preflight` either way), and a weekly readiness check that runs the `/admin/preflight` checks and posts its failures to Slack (`src/admin/readinessAlert.ts`), plus a weekly all-clear while `READINESS_POST_WHEN_HEALTHY` is `"true"` -- so a pass certificate nearing expiry is noticed without anyone opening the page. **Both environments run all five on a schedule** (`[triggers]` and `[env.staging.triggers]` in `wrangler.toml`): the Slack sync and the resync every six hours, a quarter-hour apart, the pass refresh daily at 00:30 UTC, the readiness check weekly, and the operational watch hourly. Staging's run against the sandbox store and staging's own Slack app.
...
- **New-order card emails** (`src/email/newOrder.ts`): when an order webhook reports an order that counts as a membership -- any of the paid statuses -- the member is emailed their card, once. On in both environments, for orders created on or after `CARD_EMAIL_NEW_ORDERS_SINCE`; emptying that var switches it off -- see [`docs/bigcommerce-ingestion.md`](docs/bigcommerce-ingestion.md).
