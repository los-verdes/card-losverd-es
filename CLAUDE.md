...
- **workerd `Constructing a Response with a null body status (204) and a non-null, zero-length body`** (seen once in a full `vitest run`, 2026-09-20). Not yet attributed to a spec: it does not reproduce when `test/passkit/{routes,updates,apns}.spec.ts` or `test/member/portal.spec.ts` are run on their own, and the only 204 this repo constructs (`src/passkit/routes.ts`) already passes `null`. Most likely a framework or a mock constructing `""` instead. Harmless; noted so whoever sees it next starts from here rather than from scratch.
