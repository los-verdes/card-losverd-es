...
- **`npm warn deprecated eslint@9.39.5`** (seen in every CI run that does `npm ci`). Upstream eslint 9.39.5 is past its support window per eslint.org's version-support page. Not urgent -- CI still passes -- but eslint 10 is now released, and that bump is worth scheduling deliberately (flat-config or rule behavior could shift) rather than doing reactively later. Dependabot is configured not to raise eslint majors for exactly this reason, so it will not happen on its own.
