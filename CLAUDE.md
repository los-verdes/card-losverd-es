...
- **Deliberately-held majors, checked 2026-09-19:** Vitest 5 is blocked until `@cloudflare/vitest-plugin` accepts it (1.1.13 pins `vitest ^4.1.0`), and TypeScript 7 until `typescript-eslint` does (8.70 stops at `<6.1`). Neither is a decision yet; re-check those two peer ranges before trying either. eslint is on 10 as of the same date.
